...
std::pair<float, std::array<DenoiseParameters, 5>> LeicaQ2DenoiseParameters(int iso) {
    ...
    const float min_green_variance = LeicaQ2[0].rawNlf[1];
    const float max_green_variance = LeicaQ2[LeicaQ2.size()-1].rawNlf[1];
    ...
    std::cout << "LeicaQ2DenoiseParameters nlf_alpha: " << nlf_alpha << std::endl;
    ...
    std::array<DenoiseParameters, 5> denoiseParameters = {{
        ...
        {
            .luma = 1.0f * std::lerp(1.0f, 2.0f, nlf_alpha),
            .chroma = std::lerp(1.0f, 8.0f, nlf_alpha),
