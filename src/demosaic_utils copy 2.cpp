    ...
    for (int y = 0; y < vTiles; y++) {
        for (int x = 0; x < hTiles; x++) {
            wbGains[y][x] = threadPool.enqueue([x, y, tileWidth, tileHeight, rgb_ycbcr, white, black, bayerPattern, &rawImage]() -> WhiteBalanceStats {
                ...
                return autoWhiteBalanceKernel(rawTile, rgb_ycbcr, white, black, bayerPattern);
            });
        }
    }
    for (int y = 0; y < vTiles; y++) {
        for (int x = 0; x < hTiles; x++) {
            ...
            std::cout << "wbGain(" << x << ":" << y << "): " << wb.wbGain / wb.whitePixelsCount
                      << ", diffAverage: " << wb.diffAverage << std::endl;

            /* if (wb.diffAverage > 0.005) */ {
                wbGain += wb.wbGain / wb.whitePixelsCount;
                // wbCount += wb.whitePixelsCount;
                wbCount++;
            }
        }
    }
